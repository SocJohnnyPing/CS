...
[1G 到 5G 的艱辛歷程：一部波瀾壯闊的行動通訊史](https://www.stockfeel.com.tw/1g-%E5%88%B0-5g-%E7%9A%84%E8%89%B1%E8%BE%9B%E6%AD%B7%E7%A8%8B%EF%BC%9A%E4%B8%80%E9%83%A8%E6%B3%A2%E7%80%BE%E5%A3%AF%E9%97%8A%E7%9A%84%E8%A1%8C%E5%8B%95%E9%80%9A%E8%A8%8A%E5%8F%B2/)  
   ...
   * 第三代行動通訊網路 (3G)  
      * 語音部份仍採線路交換
      * 數據資料採封包交換
      * 數據資料的傳輸速度大幅提升，也使得使用者可以接受手機上網的速度  
   ...
   * 第四代行動通訊網路 (4G)  
     * LTE技術
     * 捨棄了線路交換，完全採用封包交換– 所謂的 All IP Network
     * 聲音傳輸部份採用VoIP技術
     * 可向下相容，所以實際通話部份不是一定要使用VoIP來進行
     * 規格訂定的下載速度高達300Mbps  

   * 第五代行動通訊網路 (5G): 5G的願景裡其實訂定了三種不同的應用情境:  
...
* 使用不需要申請的ISM無線頻段
* 大多為2.4G或是5G的頻段
* 802.11標準: 俗稱的無線網路所使用的規格
 ![image](https://user-images.githubusercontent.com/91866985/143773039-b0a25a9f-1542-4ca2-aba1-9537463b2aab.png)  
「通訊距離」是指無障礙物的情況  
「傳輸速率」，是指頻寬為20MHz，且僅使用單通道情況下的理論值上限。  
...
* 主要目的是取代週邊低速裝置的連接線
* 印表機、鍵盤、滑鼠、並列埠、序列埠等
* 1998年提出，頻寬僅1Mbps，使用2.4GHz的頻段
* 新版本的規格提升頻寬至3Mbps
* 若頻寬仍不夠，可搭配802.11無線網路標準使用  

4. 無線射頻識別 – RFID  
* 無線的條碼
* 透過特殊的讀取裝置，使用無線的方式讀取
* 不需要對準條碼就可讀取，甚至可以遠距(數公尺)讀取
* RFID運作方式:  
   * 被動式
     * 沒有電源
     * 由讀取訊號天線，將電磁波轉為電能運作  
